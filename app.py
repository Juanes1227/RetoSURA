...
if st.session_state['datos_procesados'] is not None:
    ...
    if poliza_seleccionada:
        if poliza_seleccionada in lista_polizas:
            ...
            with col1:
                st.metric(label="TPR del Producto (2025)", value=f"{datos_poliza['TPR'] * 100:.4f}%")
            with col2:
            ...
            with col3:
                ...
                st.metric(label="Tasa Credibilizada", value=f"{datos_poliza['tasa_cred'] * 100:.4f}%", 
                          delta=f"{diferencia_tasa * 100:.4f}% vs TUPR", delta_color="inverse")
            with col4:
                st.metric(label="Tasa Cred. Comercial", value=f"{datos_poliza['tasa_cred_com'] * 100:.4f}%")
